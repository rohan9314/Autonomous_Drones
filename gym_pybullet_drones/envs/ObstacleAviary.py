...
class ObstacleAviary(BaseRLAviary):
    ...
    def reset(self, seed=None, options=None):
        ...
        for _ in range(200):
            ...
            if np.linalg.norm(tgt_xy - self._start_xy) >= 2.0:
                break
        tgt_z = self._rng.uniform(0.3, 2.5)
        self.TARGET_POS = np.array([tgt_xy[0], tgt_xy[1], tgt_z])
    ...
    def _addObstacles(self):
        ...
        for _ in range(self.NUM_OBSTACLES):
            for _attempt in range(100):
                ...
                # 75% chance obstacle is active in arena; otherwise parked above LiDAR range
                z_pos = float(hz) if self._rng.random() < 0.75 else 4.5
                body = p.createMultiBody(
                    baseMass=0,
                    baseCollisionShapeIndex=col,
                    baseVisualShapeIndex=viz,
                    basePosition=[float(xy[0]), float(xy[1]), z_pos],
                    physicsClientId=self.CLIENT,
                )
    ...
    def _computeInfo(self):
        ...
        start_pos = np.array([self._start_xy[0], self._start_xy[1], 0.5])
        dist_from_start = np.linalg.norm(pos - start_pos)
        efficiency = dist_from_start / max(1, self.step_counter/self.CTRL_FREQ)
        return {
