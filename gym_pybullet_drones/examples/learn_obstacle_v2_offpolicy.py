...
class SuccessRateCurriculumCallback(BaseCallback):
    ...
    def __init__(self, eval_freq, save_path, max_difficulty=MAX_DIFFICULTY, verbose=1):
...
def run(
    ...
    curriculum_cb = SuccessRateCurriculumCallback(eval_freq=1000, save_path=save_dir, verbose=1)
    curriculum_cb.difficulty = difficulty

    print(f"[INFO] Training {algo.upper()} for {int(total_timesteps):,} steps over {n_envs} envs")
    model.learn(total_timesteps=int(total_timesteps), callback=curriculum_cb, log_interval=50)
